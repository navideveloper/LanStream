...
def main(page: ft.Page):
    ...
    status_indicator = ft.Row([
        ft.Text('Online' if settings["status"] else "Offline",color=ft.Colors.WHITE,size=18),
        ft.Container(
            width=15,
            height=15,
            border_radius=50,
            bgcolor=ft.Colors.GREEN_700 if settings["status"] else ft.Colors.RED_700,
            tooltip="Online" if settings["status"] else "Offline"
        ),
    ])
    ...
    page.add(
        ft.Container(
            ft.Column([
                ...
                ft.Container(
                    ft.Row([
                        ft.Text(settings["ip"], size=24, weight='w700', color="#D1D1D1",font_family='consolas'),
                        status_indicator
                    ], alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
                ...
                ft.Row([
                    ModernContainer(
                        "Status",
                        ft.Row([ft.Text("Offline" if not settings["status"] else "Online", color=ft.Colors.GREY_400), streaming_switch])
                    ),
                    ModernContainer(
                        "Port",
                        port_field
